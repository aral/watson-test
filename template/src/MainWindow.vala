namespace WatsonTest {
    public class MainWindow : Hdy.ApplicationWindow {
        public weak WatsonTest.Application app { get; construct; }

        // Widgets
        public WatsonTest.Widgets.HeaderBar toolbar;
        ...
        public MainWindow (WatsonTest.Application application) {
            Object (
                ...
                icon_name: "com.github.aral.watson_test"
            );
        }
        ...
        private void create_layout () {
            ...
            toolbar = new WatsonTest.Widgets.HeaderBar ();
        ...
        private void restore_window_state () {
            ...
            WatsonTest.saved_state.get ("window-size", "(ii)", out rect.width, out rect.height);

            default_width = rect.width;
            default_height = rect.height;

            var window_state = WatsonTest.saved_state.get_enum ("window-state");
            switch (window_state) {
                ...
                case WindowState.FULLSCREEN:
                    ...
                    break;
                default:
                    WatsonTest.saved_state.get ("window-position", "(ii)", out rect.x, out rect.y);
        ...
        private void preserve_window_state () {
            ...
            if (Gdk.WindowState.MAXIMIZED in state) {
                WatsonTest.saved_state.set_enum ("window-state", WindowState.MAXIMIZED);
            } else if (Gdk.WindowState.FULLSCREEN in state) {
                WatsonTest.saved_state.set_enum ("window-state", WindowState.FULLSCREEN);
            } else {
                WatsonTest.saved_state.set_enum ("window-state", WindowState.NORMAL);
                // Save window size
                int width, height;
                get_size (out width, out height);
                WatsonTest.saved_state.set ("window-size", "(ii)", width, height);
            }

            int x, y;
            get_position (out x, out y);
            WatsonTest.saved_state.set ("window-position", "(ii)", x, y);
        }
